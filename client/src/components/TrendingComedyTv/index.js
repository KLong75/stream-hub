...
const TrendingComedyTv = () => {
  const [trendingComedyTv, setTrendingComedyTv] = useState([]);
  console.log(trendingComedyTv);
  ...
  useEffect(() => {
    const getTrendingComedyTv = async () => {
      const cachedTrendingComedyTv = localStorage.getItem(
        "trendingComedyTv"
      );

      if (cachedTrendingComedyTv) {
        const { data, timestamp } = JSON.parse(cachedTrendingComedyTv);
        console.log("Cached Data Retrieved: cachedTrendingComedyTv", data);
        const now = Date.now();
        if (now - timestamp < CACHE_DURATION_ONE_WEEK) {
          setTrendingComedyTv(data);
          return;
        } else {
          localStorage.removeItem("trendingComedyTv");
      ...
      if (!cachedTrendingComedyTv) {
        try {
          const response = await fetchTrendingComedyTv();
          ...
          setTrendingComedyTv(topTvShows);
  ...
  return (
    <>
      <h3>Trending Comedy TV Shows</h3>
      <div>
        {trendingComedyTv.map((tvShow) => (
          <div key={tvShow.id}>
            <img
...
export default TrendingComedyTv;