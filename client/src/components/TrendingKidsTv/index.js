...
const TrendingKidsTv = () => {
  const [trendingKidsTv, setTrendingKidsTv] = useState([]);
  console.log(trendingKidsTv);
  ...
  useEffect(() => {
    const getTrendingKidsTv = async () => {
      const cachedTrendingKidsTv = localStorage.getItem(
        "trendingKidsTv"
      );

      if (cachedTrendingKidsTv) {
        const { data, timestamp } = JSON.parse(cachedTrendingKidsTv);
        console.log("Cached Data Retrieved: cachedTrendingKidsTv", data);
        const now = Date.now();
        if (now - timestamp < CACHE_DURATION_ONE_WEEK) {
          setTrendingKidsTv(data);
          return;
        } else {
          localStorage.removeItem("trendingKidsTv");
      ...
      if (!cachedTrendingKidsTv) {
        try {
          const response = await fetchTrendingKidsTv();
