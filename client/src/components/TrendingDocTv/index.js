...
const TrendingDocTv = () => {
  const [trendingDocTv, setTrendingDocTv] = useState([]);
  console.log(trendingDocTv);
  ...
  useEffect(() => {
    const getTrendingDocTv = async () => {
      const cachedTrendingDocTv = localStorage.getItem(
        "trendingDocTv"
      );

      if (cachedTrendingDocTv) {
        const { data, timestamp } = JSON.parse(cachedTrendingDocTv);
        console.log("Cached Data Retrieved: cachedTrendingDocTv", data);
        const now = Date.now();
        if (now - timestamp < CACHE_DURATION_ONE_WEEK) {
          setTrendingDocTv(data);
          return;
        } else {
          localStorage.removeItem("trendingDocTv");
      ...
      if (!cachedTrendingDocTv) {
        try {
          const response = await fetchTrendingDocTv();
