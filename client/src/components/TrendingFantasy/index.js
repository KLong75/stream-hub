...
const TrendingFantasy = () => {
  const [trendingFantasyMovies, setTrendingFantasyMovies] = useState([]);
  console.log(trendingFantasyMovies);
  ...
  useEffect(() => {
    const getTrendingFantasyMovies = async () => {
      const cachedTrendingFantasyMovies = localStorage.getItem(
        "trendingFantasyMovies"
      );

      if (cachedTrendingFantasyMovies) {
        const { data, timestamp } = JSON.parse(cachedTrendingFantasyMovies);
        console.log("Cached Data Retrieved: cachedTrendingFantasyMovies", data);
        const now = Date.now();
        if (now - timestamp < CACHE_DURATION_ONE_WEEK) {
          setTrendingFantasyMovies(data);
          return;
        } else {
          localStorage.removeItem("trendingFantasyMovies");
      ...
      if (!cachedTrendingFantasyMovies) {
        try {
          const response = await fetchTrendingFantasyMovies();
          ...
          const filteredTrendingFantasyMovies = allMovies.filter((movie) =>
            movie.genre.includes(14)
          );

          const filteredOutMovies = allMovies.filter(
            (movie) => !movie.genre.includes(14)
          );
          console.log(
            "Filtered out movies: ",
          ...
          setTrendingFantasyMovies(filteredTrendingFantasyMovies);

          const cacheData = {
            data: filteredTrendingFantasyMovies,
            timestamp: Date.now(),
          };
  ...
  return (
    <>
      <h3>Trending Fantasy Movies</h3>
      <div>
        {trendingFantasyMovies.map((movie) => (
          <div key={movie.id}>
            <img
...
export default TrendingFantasy;